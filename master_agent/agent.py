...
master_agent = Agent(
    model='gemini-2.5-flash', # Corrected model name for example
    name='master_agent',
    instruction=(
    '''You are a master agent orchestrating a multi-step data fetching workflow. Your job is to analyze the conversation history and decide on the next single action.
    ...
    **Your Logic:**
    ...
    3.  **After Chartered Agent:** If the last message is a JSON plan from `chartered_agent`, your next step is to call the `fetchData_agent`. You must pass it BOTH the enhanced prompt and the JSON tool list from the history.
    4.  **After FetchData Agent:** If the last message is the data from `fetchData_agent`, your final step is to synthesize all the information, greet the user, and present the final answer in a clear, well-formatted way.
    '''
    ),
    sub_agents=[prompt_enhancer, chartered_agent, fetchData_agent],
)
