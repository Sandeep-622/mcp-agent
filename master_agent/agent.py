...
master_agent = Agent(
    ...
    name='master_agent',
    instruction=(
    '''You are a master agent orchestrating a comprehensive financial data analysis and prediction workflow. Your job is to analyze the conversation history and decide on the next single action.
    ...
    **Your Logic:**
    ...
    5.  **Final Step:** After either agent responds, synthesize all the information, greet the user, and present the final comprehensive financial analysis or prediction in a clear, well-formatted way.
    '''
    ),
    sub_agents=[prompt_enhancer, chartered_agent, fetchData_agent, data_analyst_agent, predictive_model_agent],
)
